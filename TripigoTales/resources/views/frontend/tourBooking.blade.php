            ...
            <div class="row flex-column-reverse flex-lg-row">
                                    ...
                                    <div class="row g-4">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name" value="{{ $user->name ?? '' }}" placeholder="Full Name" readonly class="form-control">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="form-label-custom">Email</label>
                                                <input type="email" name="email" value="{{ $user->email ?? '' }}" placeholder="Email Address" readonly class="form-control">
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="tel" id="placeholder_main_phone" name="phone"
                                                        value="{{ preg_replace('/^(\+91|91|0)/', '', $user->phone ?? '') }}"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" id="from_date" name="from_date" value="{{ $tour->friday_date }}">
                                                <input type="hidden" id="to_date" name="to_date">
                                                <input type="hidden" id="days" name="days" value="{{ (int)$tour->tour_duration }}">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select id="persons" name="persons" required class="form-select">
                                                    ...
                                                    @for($p = 1; $p <= min(10, $tour->max_people ?? 10); $p++)
                                                        <option value="{{ $p }}">{{ $p }} {{ $p == 1 ? 'Person' : 'Persons' }}</option>
                                                    @endfor
                                                </select>
                                            </div>
                                        </div>
                                                ...
                                                <div class="d-flex justify-content-between align-items-center bg-light p-2 rounded border">
                                                    <div>
                                                        <div class="form-check mb-0">
                                                            <input class="form-check-input addon-checkbox" type="checkbox" id="addon_{{ $addon->id }}" value="{{ $addon->id }}" data-price="{{ $addon->price }}">
                                                            <label class="form-check-label fw-bold" for="addon_{{ $addon->id }}">
                                                                {{ $addon->name }} <span class="text-success ms-1">(+₹{{ number_format($addon->price, 0) }})</span>
                                                            </label>
                                                        </div>
                                                        @if($addon->description)
                                                            <small class="text-muted d-block ms-4" style="line-height: 1.2;">{{ $addon->description }}</small>
                                                        @endif
                                                    </div>
                                                    <div class="addon-qty-container ms-2" style="display:none; flex-shrink: 0;">
                                                        <div class="addon-qty-group">
                                                            <button type="button" class="addon-qty-btn minus-btn" data-id="{{ $addon->id }}">-</button>
                                                            <input type="number" class="addon-qty-input addon-qty" name="addons[{{ $index }}][quantity]" min="1" value="1" data-id="{{ $addon->id }}" readonly>
                                                            <button type="button" class="addon-qty-btn plus-btn" data-id="{{ $addon->id }}">+</button>
                                                        </div>
                                                        <input type="hidden" name="addons[{{ $index }}][id]" value="{{ $addon->id }}" class="addon-id-input" disabled>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                ...
                                <table class="w-100 mb-3">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="theme2 text-end py-1">₹{{ number_format($tour->price, 2) }}</td>
                                        </tr>
                                        <tr>
                                ...
                                <div class="booking-terms border-t pt-3 mt-3">
                                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between">
                                        <div class="form-group mb-3 mb-lg-0 w-100 pe-lg-3">
                                            ...
                                            <a href="{{ route('frontend.terms.and.conditions') }}" class="text-theme fw-semibold">Terms</a> and
                                            <a href="{{ route('frontend.cancellation.policy') }}" class="text-theme fw-semibold">Cancellation Policy</a>.</small>
                                        </div>
                ...
                <div class="col-lg-4 mb-4 ps-ld-4">
                    <div class="sidebar-sticky">
                                ...
                                <div class="row">
                                    <div class="col-lg-4 col-md-4">
                                       <div class="trend-item2 rounded">
                                            <a href="{{ route('frontend.tour.detail', $tour->slug) }}" style="background-image: url({{ $tour->image ? asset($tour->image) : asset('images/destination/destination17.jpg') }});"></a>
                                    ...
                                    <div class="col-lg-8 col-md-8 ps-0">
                                        <div class="trend-content position-relative">
                                            <div class="rating mb-1">
                                                ...
                                                @endfor
                                                <small>{{ $reviewCount }} {{ Str::plural('Review', $reviewCount) }}</small>
                                            </div>
                                            <h5 class="mb-1"><a href="{{ route('frontend.tour.detail', $tour->slug) }}">{{ $tour->title }}</a></h5>
                                            <h6 class="theme mb-0"><i class="icon-location-pin"></i> {{ $tour->location ?? 'N/A' }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            @if($tour->category)
                                <div class="mb-1"><small><i class="fa fa-tag"></i> {{ $tour->category->name }}</small></div>
                            @endif
                            @if($tour->tour_duration || $tour->day)
                                <div class="mb-1"><small><i class="fa fa-clock"></i> Duration: {{ $tour->tour_duration ?? (is_array($tour->day) ? count($tour->day) : $tour->day) }} Days</small></div>
                            @endif
                            @if($tour->max_people)
                                <div class="mb-1"><small><i class="fa fa-users"></i> Max People: {{ $tour->max_people }}</small></div>
                            @endif
                            @if($tour->pickup)
                                <div class="mb-1"><small><i class="fa fa-map-marker-alt"></i> Pickup: {{ $tour->pickup }}</small></div>
                            @endif
                            @if($tour->departure_time)
                                <div class="mb-1"><small><i class="fa fa-plane-departure"></i> Departure: {{ $tour->departure_time }}</small></div>
                            @endif
                            @if($tour->language)
                                <div class="mb-1"><small><i class="fa fa-language"></i> Language: {{ $tour->language }}</small></div>
                            @endif
                        </div>
                        ...
                        @if($tour->discount_status && $tour->special_discount > 0)
                        <!-- Discount Card -->
                        <div class="sidebar-item bg-white rounded box-shadow overflow-hidden p-3 mb-4">
                            <h4 class="text-success"><i class="fa fa-tag"></i> Special Offer</h4>
                            <p class="mb-0">{{ $tour->special_discount }}% discount available on this tour!</p>
                        </div>
                        @endif
...
(function () {
    ...
    function triggerCalculatePrice(isCouponAction = false) {
        ...
        debounceTimer = setTimeout(() => {
            ...
            fetch("{{ route('booking.calculate.price') }}", {
                method: 'POST',
                headers: {
    ...
    const scheduleType  = @json($scheduleType);
    const scheduleDays  = @json($scheduleDays);
    const specificDates = @json($specificDates);
    ...
    document.querySelectorAll('#pay-with-razorpay, #pay-with-razorpay-mobile').forEach(btn => {
        btn.addEventListener('click', function (e) {
            ...
            const formData = new FormData(bookingForm);
            fetch("{{ route('booking.razorpay.order') }}", {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
            ...
            .then(function (res) {
                ...
                var options = {
                    ...
                    handler: function (response) {
                        ...
                        fetch("{{ route('booking.razorpay.verify') }}", {
                            method: 'POST',
                            headers: {
                        ...
                        .then(function (v) {
                            if (v.success) {
                                ...
                                window.location.href = v.redirectTo || "{{ route('frontend.tour.detail', $tour->slug) }}";
                            } else {
