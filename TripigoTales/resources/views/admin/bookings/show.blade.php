    ...
    <div class="booking-header">
        <div>
            ...
            <div class="booking-meta">
                <i class="fa fa-clock-o me-1"></i> Booked on {{ $booking->created_at->format('d M Y, h:i A') }}
                &nbsp;·&nbsp;
                <i class="fa fa-map-marker me-1"></i> {{ $booking->tour->location ?? 'Tour Deleted' }}
            </div>
        </div>
        <div class="d-flex align-items-center gap-3 no-print">
            ...
            <a href="{{ route('admin.bookings.invoice', $booking->slug) }}" class="btn btn-success btn-sm">
                <i class="fa fa-file-pdf-o me-1"></i> Download Invoice
            </a>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    ...
                    <div class="px-2">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $booking->tour->location ?? '—' }}</span>
                        </div>
                        <div class="info-row">
                            ...
                            <span class="info-value">
                                ...
                                @if($booking->to_date)
                                    &nbsp;→&nbsp; {{ $booking->to_date->format('d M Y') }}
                                @endif
                            </span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Duration</span>
                            <span class="info-value">{{ $booking->days ?? ($booking->tour->tour_duration ?? 'N/A') }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">No. of Persons</span>
                            <span class="info-value">
                                <strong>{{ $booking->persons }}</strong> traveller(s)
                            </span>
                        </div>
                        ...
                        <div class="info-row mt-3 bg-light p-3 rounded">
                            ...
                            <span class="info-value">
                                <div class="text-danger fw-bold">Cancelled on {{ $booking->cancelled_at->format('d M Y, h:i A') }}</div>
                                <div class="mt-1"><strong>Reason:</strong> {{ $booking->cancellation_reason ?? 'N/A' }}</div>
                            </span>
                        </div>
                        @endif
                        ...
                        <div class="info-row mt-2 bg-light p-3 rounded">
                            ...
                            <span class="info-value">
                                <div class="text-info fw-bold">Refund ID: {{ $booking->refund_id }}</div>
                                <div class="mt-1"><strong>Amount:</strong> ₹{{ number_format($booking->refund_amount, 2) }}</div>
                                <div class="mt-1"><strong>Status:</strong> {{ ucfirst($booking->refund_status) }}</div>
                            </span>
                        </div>
                        @endif
                    </div>
            ...
            <div class="card border-0 rounded-3 shadow-sm mb-4">
                <div class="card-body">
                    <div class="section-header mb-3">
                        <i class="fa fa-users"></i> Traveller Details ({{ $booking->travelers->count() }})
                    </div>
                            ...
                            <table class="table traveler-table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $traveler->dob ? \Carbon\Carbon::parse($traveler->dob)->format('d M Y') : '—' }}</td>
                    ...
                    <div class="table-responsive">
                        <table class="table traveler-table mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><strong>{{ $ba->addon->name ?? 'Unknown Add-On' }}</strong></td>
                                    <td class="text-center">× {{ $ba->quantity }}</td>
                                    <td class="text-end">₹{{ number_format($ba->price / max($ba->quantity, 1), 2) }}</td>
                                    <td class="text-end fw-semibold">₹{{ number_format($ba->price, 2) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="card border-0 rounded-3 shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <div class="px-2" style="max-width:420px;">
                        <div class="price-row">
                            <span class="label">
                                Price per Person (₹{{ number_format($booking->price_per_person, 2) }} × {{ $booking->persons }})
                            </span>
                            <span class="amount">₹{{ number_format($booking->subtotal ?? ($booking->price_per_person * $booking->persons), 2) }}</span>
                        </div>

                        @if($booking->addons_amount > 0)
                        <div class="price-row">
                            <span class="label">Add-Ons</span>
                            <span class="amount">+ ₹{{ number_format($booking->addons_amount, 2) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="price-row discount">
                            ...
                            <span class="amount">− ₹{{ number_format($booking->discount_amount, 2) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="price-row">
                            ...
                            <span class="amount">+ ₹{{ number_format($booking->gst_amount, 2) }}</span>
                        </div>
                        @endif
                    ...
                    <div class="px-2">
                        <div class="info-row">
                            ...
                            <span class="info-value"><code class="ref">{{ $booking->razorpay_order_id ?? 'N/A' }}</code></span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Razorpay Payment ID</span>
                            <span class="info-value"><code class="ref">{{ $booking->razorpay_payment_id ?? 'N/A' }}</code></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="sidebar-card card mb-4 no-print">
                ...
                <div class="card-body">
                    <div class="text-center mb-3">
                        <span class="status-badge {{ $statusClass }}" style="font-size:15px;padding:8px 24px;">
                            {{ $statusIcon }} {{ ucfirst($booking->status) }}
                        </span>
                    </div>
                    <form action="{{ route('admin.bookings.updateStatus', $booking->slug) }}" method="POST">
                        @csrf
                        <select name="status" class="form-control mb-3" style="border-radius:8px;font-size:14px;">
                            @foreach(['pending','paid','failed','cancelled','refunded', 'awaiting_refund'] as $s)
                    ...
                    <hr>
                    <form action="{{ route('admin.bookings.refund', $booking->slug) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-success w-100"
                                onclick="return confirm('Initiate Razorpay Refund for ₹{{ number_format($booking->total_amount, 2) }}?')">
            ...
            <div class="sidebar-card card mb-4">
                ...
                <div class="card-body">
                    <div class="info-row" style="padding:7px 0;">
                        ...
                        <span class="info-value fw-semibold">#{{ $booking->id }}</span>
                    </div>
                    <div class="info-row" style="padding:7px 0;">
                    ...
                    <div class="info-row" style="padding:7px 0;">
                        ...
                        <span class="info-value">{{ $booking->from_date?->format('d M Y') ?? 'N/A' }}</span>
                    </div>
                    <div class="info-row" style="padding:7px 0;">
                    ...
                    <div class="info-row" style="padding:7px 0;">
                        ...
                        <span class="info-value fw-bold text-primary">₹{{ number_format($booking->total_amount, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="sidebar-card card border-danger no-print">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.bookings.destroy', $booking->slug) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-outline-danger w-100"
                                onclick="return confirm('Permanently delete Booking #{{ $booking->id }}? This cannot be undone.')">
